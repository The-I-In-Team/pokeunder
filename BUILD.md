...
These instructions explain how to set up the tools required to build **Pokémon: Underground Version**, which assembles the source files into a ROM.
...
## Windows 10/11 (WSL1)
WSL1 is the preferred terminal to build **Pokémon: Underground Version**. The following instructions will explain how to install WSL1 (referred to interchangeably as WSL).
...
- Otherwise, **open WSL** and go to [Choosing where to store Pokémon: Underground Version (WSL1)](#Choosing-where-to-store-Pokémon:-Underground-Version-WSL1).
...
### Choosing where to store Pokémon: Underground Version (WSL1)
WSL has its own file system that's not natively accessible from Windows, but Windows files *are* accessible from WSL. So you're going to want to store Pokémon: Underground Version within Windows.

For example, say you want to store Pokémon: Underground Version (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps**. First, ensure that the folder already exists. Then, enter this command to **change directory** to said folder, where *\<user>* is your **Windows** username:
...
- Otherwise, **open msys2** and go to [Choosing where to store Pokémon: Underground Version (msys2)](#choosing-where-to-store-Pokémon: Underground Version-msys2).
...
2. Certain packages are required to build Pokémon: Underground Version. Install these by running the following two commands:
...
### Choosing where to store Pokémon: Underground Version (msys2)
At this point, you can choose a folder to store Pokémon: Underground Version into. If you're okay with storing Pokémon: Underground Version in the user profile folder, then proceed to [Installation](#installation). Otherwise, you'll need to account for where Pokémon: Underground Version is stored when changing directory to the Pokémon: Underground Version folder.

For example, if you want to store Pokémon: Underground Version (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps** (where *\<user>* is your **Windows** username), enter this command:
...
2.
    ...
    - Otherwise, **open Cygwin** and go to [Choosing where to store Pokémon: Underground Version (Cygwin)](#choosing-where-to-store-Pokémon: Underground Version-cygwin)
...
### Choosing where to store Pokémon: Underground Version (Cygwin)

Cygwin has its own file system that's within Windows, at **C:\cygwin64\home\\_\<user>_**. If you don't want to store Pokémon: Underground Version there, you'll need to account for where Pokémon: Underground Version is stored when **changing directory** to the Pokémon: Underground Version folder.

For example, if you want to store Pokémon: Underground Version (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps**, enter this command, where *\<user>* is your **Windows** username:
```bash
cd c:/Users/<user>/Desktop/decomps
```
Note that the directory **must exist** in Windows. If you want to store Pokémon: Underground Version in a dedicated folder that doesn't exist (e.g. the example provided above), then create the folder (e.g. using Windows Explorer) before executing the `cd` command.
...
2.  - If libpng is **not installed**, then go to [Installing libpng (macOS)](#installing-libpng-macos).
    ...
    - Otherwise, **open the Terminal** and go to [Choosing where to store Pokémon: Underground Version (macOS)](#choosing-where-to-store-Pokémon: Underground Version-macos)
...
3. Run the following command to install libpng.
    ...
    Continue to [Installing devkitARM (macOS)](#installing-devkitarm-macos) if **devkitARM is not installed**, otherwise, go to [Choosing where to store Pokémon: Underground Version (macOS)](#choosing-where-to-store-Pokémon: Underground Version-macos).
...
### Choosing where to store Pokémon: Underground Version (macOS)
At this point, you can choose a folder to store Pokémon: Underground Version into. If you're okay with storing Pokémon: Underground Version in the user folder, then proceed to [Installation](#installation). Otherwise, you'll need to account for where Pokémon: Underground Version is stored when changing directory to the Pokémon: Underground Version folder.

For example, if you want to store Pokémon: Underground Version (and agbcc) in **~/Desktop/decomps**, enter this command to **change directory** to the desired folder:
```bash
cd Desktop/decomps
```
Note that the directory **must exist** in the folder system. If you want to store Pokémon: Underground Version in a dedicated folder that doesn't exist (e.g. the example provided above), then create the folder (e.g. using Finder) before executing the `cd` command.
...
sudo apt install build-essential binutils-arm-none-eabi git libpng-dev
```
Then proceed to [Choosing where to store Pokémon: Underground Version (Linux)](#choosing-where-to-store-Pokémon: Underground Version-linux).
<details>
    <summary><i>Note for legacy repos...</i></summary>

>   If the repository you plan to build has an **[older revision of the INSTALL.md](https://github.com/pret/Pokémon: Underground Version/blob/571c598/INSTALL.md)**,
...
### Choosing where to store Pokémon: Underground Version (Linux)
At this point, you can choose a folder to store Pokémon: Underground Version (and agbcc) into. If so, you'll have to account for the modified folder path when changing directory to the Pokémon: Underground Version folder.
...
>   Consider adding an exception for the `Pokémon: Underground Version` and/or `decomps` folder in Windows Security using
...
1. If Pokémon: Underground Version is not already downloaded (some users may prefer to download Pokémon: Underground Version via a git client like GitHub Desktop), run:

    ```bash
    git clone https://github.com/pret/Pokémon: Underground Version
    ```
    ...
    >   cd <folder where Pokémon: Underground Version is to be stored>
    >   ```
    >   Where *\<folder where Pokémon: Underground Version is to be stored>* is the path of the folder [where you chose to store Pokémon: Underground Version](#Choosing-where-to-store-Pokémon: Underground Version-WSL1). Then run the `git clone` command again.
    </details>

2. Install agbcc into Pokémon: Underground Version. The commands to run depend on certain conditions. **You should only follow one of the listed instructions**:
- If agbcc has **not been built before** in the folder where you chose to store Pokémon: Underground Version, run the following commands to build and install it into Pokémon: Underground Version:

    ```bash
    git clone https://github.com/pret/agbcc
    cd agbcc
    ./build.sh
    ./install.sh ../Pokémon: Underground Version
    ```

- **Otherwise**, if agbcc has been built before (e.g. if the git clone above fails), but was **last built on a different terminal** than the one currently used (only relevant to Windows, e.g. switching from msys2 to WSL1), then run the following commands to build and install it into Pokémon: Underground Version:
...
- **Otherwise**, if agbcc has been built before on the same terminal, run the following commands to install agbcc into Pokémon: Underground Version:
    ...
    <details>
        ...
        > If building agbcc or Pokémon: Underground Version results in an error, try deleting the agbcc folder and re-installing agbcc as if it has not been built before.
    </details>

3. Once agbcc is installed, change directory back to the base directory where Pokémon: Underground Version and agbcc are stored:

    ```bash
    cd ..
    ```

Now you're ready to [build **Pokémon: Underground Version**](#build-Pokémon: Underground Version)
## Build Pokémon: Underground Version
If you aren't in the Pokémon: Underground Version directory already, then **change directory** to the Pokémon: Underground Version folder:
```bash
cd Pokémon: Underground Version
```
To build **Pokémon: Underground Version.gba** (Note: to speed up builds, see [Parallel builds](#parallel-builds)):
```bash
make
```
If it has built successfully you will have the output file **Pokémon: Underground Version.gba** in your project folder.
<details>
...
```bash
Pokémon: Underground Version.gba: OK
```
If there are any changes from the original game, you will instead see:
```bash
Pokémon: Underground Version.gba: FAILED
...
To build **Pokémon: Underground Version.elf** with debug symbols under a modern toolchain:
```bash
make modern DINFO=1
```
